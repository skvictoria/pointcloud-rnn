...
batch_size = 8
display_iter = 100  # show test set accuracy during training


def extract_batch_size(_train, step, batchsize):
    ...
    for i in range(batchsize):
        # Loop index
        index = ((step - 1) * batchsize + i) % len(_train)
        batch_s[i] = _train[index]

    return batch_s


def one_hot(y_, num_classes=n_classes):
    y_ = y_.reshape(len(y_))
    return np.eye(num_classes)[np.array(y_, dtype=np.int32)]  # Returns FLOATS
...
X_train = np.empty([0, cnt, 4])
X_test = np.empty([0, cnt, 4])
y_train = np.empty([0, ])
y_test = np.empty([0, ])
...
for index, row in label_df.iterrows():
    ...
    fill_lnchn = False
    tracknum = index
    if not np.isnan(row['start_turn']):
        turn = (1, row['start_turn'], row['fin_turn'])
        fill_turn = True
    if not np.isnan(row['start_lnchn']):
        lanechng = (1, row['start_lnchn'], row['fin_lnchn'])
        fill_lnchn = True
    if(fill_lnchn == False): lanechng = (0,0,0)
    if(fill_turn == False): turn = (0,0,0)

    print(lanechng, turn)
    tmp_dataframe = pd.concat([tmp_dataframe, dataAssembly(tracknum,lanechng, turn,'./data/')])
